# Grotto Cannon Prop Set — Roblox Spec Doc

**Asset:** grotto_cannon_prop_set  
**Category:** Props  
**File:** `grotto_cannon_prop_set.blend` / `grotto_cannon_prop_set.fbx`  
**Scale:** 1 Blender unit = 1 Roblox stud  
**Overall Dimensions:** ~4 W × 4.5 D × 1.8 H studs (cannon) + cannonballs at muzzle  
...
| Cannonball_Iron | `#2A2A2A` | SmoothPlastic   | Near-black iron — 4 cannonballs on the ground       |
...
| Object       | Description                                          |
|--------------|------------------------------------------------------|
| Cannonball_1 | Stack bottom-left — left of muzzle                   |
| Cannonball_2 | Stack bottom-right — left of muzzle                  |
| Cannonball_3 | Stack top ball — resting in groove of bottom two     |
| Cannonball_4 | Lone ball — right side of muzzle                     |
...
- **Wheels** at all four corners. Axles pass through cheeks at Z=0.55.
- **Trunnion pins** protrude from both sides of the barrel at the balance point.
- **Cannonball stack** — 3 balls in a pyramid (2 on ground, 1 on top) positioned to the left of the muzzle. All three are touching: center-to-center = 0.54 studs (= 2 × radius). Ball radius = 0.27 studs.
- **Lone cannonball** — single ball to the right of the muzzle.
- All cannonballs sit on the ground (Z = 0).
...
3. **Apply materials by name:**
   - `Cannon_Iron` parts → `#3A3A3A` SmoothPlastic
   - `Cannon_Wood` parts → `#6B4226` SmoothPlastic
   - `Cannon_Bore` → `#0A0A0A` SmoothPlastic (simulates hollow barrel opening)
   - `Cannonball_*` → `#2A2A2A` SmoothPlastic
4. **Object count** — ~26 objects. Consider merging by material in Studio if performance is a concern.
